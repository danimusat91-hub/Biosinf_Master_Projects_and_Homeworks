    ...
    def extractArmFeatures(self):
        ...
        for count,d in enumerate(self.dataStore):
            ...
            while start + N <= len(d[0]):
                ...
                # window1 *= sp.signal.windows.hamming(N)
                # window2 *= sp.signal.windows.hamming(N)
                # window3 *= sp.signal.windows.hamming(N)
                # window4 *= sp.signal.windows.hamming(N)
                # window5 *= sp.signal.windows.hamming(N)
                # window6 *= sp.signal.windows.hamming(N)
                # window7 *= sp.signal.windows.hamming(N)
                # window8 *= sp.signal.windows.hamming(N)
